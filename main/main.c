...
// --- Constantes de Escala e Zona Morta ---
#define ADC_MAX_VAL 4095
#define ADC_CENTER_VAL (ADC_MAX_VAL / 2) // 2047
...
// --- Task para Eixo X (ADC0) ---
void x_task(void *pvParameters) {
    adc_t adc_data;
    adc_data.axis = 0; 
    
    TickType_t xLastWakeTime = xTaskGetTickCount();
    const TickType_t xFrequency = pdMS_TO_TICKS(100); // 100ms para menor velocidade de envio

    for (;;) {
        adc_select_input(ADC_X_CHANNEL);
        uint16_t raw_adc = adc_read();
        
        // Média Móvel - 1. Atualiza o histórico
        x_history[x_idx] = raw_adc;
        x_idx = (x_idx + 1) % FILTER_SIZE;

        // Média Móvel - 2. Calcula a Média
        uint32_t sum = 0;
